...
import { RotateCcw, Plus, Minus, Move, MousePointer2, ArrowUpRight, Crosshair, Pin, X } from 'lucide-react';
...
function Neuron({selected,onSelect,onHover,anchor,lockedAnchor,connector,popup,api}:{selected:number|null;onSelect:(n:number)=>void;onHover:(n:number|null)=>void;anchor:React.MutableRefObject<Anchor|null>;lockedAnchor:React.MutableRefObject<Anchor|null>;connector:React.RefObject<SVGSVGElement|null>;popup:React.RefObject<HTMLElement|null>;api:React.MutableRefObject<{reset:()=>void;zoom:(n:number)=>void}|null>}) {
 ...
 const scene=new THREE.Scene();const camera=new THREE.PerspectiveCamera(38,1,.1,100);camera.position.set(0,0,23);const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.enablePan=false;controls.minDistance=12;controls.maxDistance=36;controls.target.set(.3,0,0);
 ...
 api.current={reset:()=>{camera.position.set(0,0,23);controls.target.set(.3,0,0);controls.update();},zoom:(n)=>{camera.position.sub(controls.target).multiplyScalar(n).clampLength(12,36).add(controls.target);controls.update();}};
 ...
 return <div className="canvas" ref={host}>{error&&<p className="render-error">3D rendering is unavailable in this browser. Try a browser with WebGL enabled. You can still explore every structure using the list below.</p>}</div>;
}
export default function Home(){
 ...
 useEffect(()=>{const escape=(e:KeyboardEvent)=>{if(e.key==='Escape')close();};window.addEventListener('keydown',escape);return()=>window.removeEventListener('keydown',escape);},[close]);
 // Keep a pinned label attached to its original hit even as the pointer explores elsewhere.

 return <main>
 <header className="hud"><div className="identity"><Crosshair size={31}/><div><span className="eyebrow">NEURAL ANATOMY INTERFACE</span><h1>NEURON<span>_01</span></h1></div></div><div className="telemetry"><span>SPECIMEN <b>MULTIPOLAR</b></span><span>STRUCTURES <b>08</b></span><span>LABEL <b>{pinned!==null?'PINNED':hovered!==null?'PREVIEW':'STANDBY'}</b></span></div><div className="hud-status"><i/> 3D EXPLORER<span>PSYCHOLOGY / STUDY SYSTEM</span></div></header>
 <section className="workspace" aria-label="Interactive neuron study model"><div className="viewer"><div className="viewer-top"><span>01 / SPECIMEN VIEW</span><span>ROTATION ENABLED</span></div><Neuron selected={selected} onSelect={pin} onHover={onHover} anchor={anchor} lockedAnchor={pinnedAnchor} connector={connector} popup={popup} api={api}/><div className="axis-mark" aria-hidden="true">Y +<br/>└── X +</div><div className="viewer-bottom"><span><Move size={16}/> Drag to rotate · Scroll to zoom</span><div className="view-controls"><button aria-label="Zoom in" onClick={()=>api.current?.zoom(.85)}><Plus size={18}/></button><button aria-label="Zoom out" onClick={()=>api.current?.zoom(1.18)}><Minus size={18}/></button><button aria-label="Reset model view" onClick={()=>api.current?.reset()}><RotateCcw size={18}/></button></div></div></div>
 ...
 <div className="label-zone" aria-live="polite">{p&&selected!==null?<aside ref={popup} className="label-popup" style={{'--accent':p.color} as React.CSSProperties} aria-label={`${p.name} information`}><div className="label-top"><span>{pinned!==null?<Pin size={14}/>:<Crosshair size={14}/>} {pinned!==null?'TARGET LOCKED':'HOVER PREVIEW'}</span><button onClick={close} aria-label="Close anatomy label"><X size={18}/></button></div><div className="label-id">STRUCTURE / 0{selected+1}</div><h2>{p.name}</h2><div className="function-label">{p.verb}</div><p>{p.text}</p><div className="relation"><h3>In the whole neuron</h3><p>{p.whole}</p></div><div className="remember"><span>FIELD NOTE</span><p>{p.note}</p></div><div className="label-foot">{pinned!==null?'Click another structure to change target.':'Click the structure to keep this label.'}</div></aside>:<div className="standby"><Crosshair size={35}/><h2>Acquire a target.</h2><p>Hold the cursor still over a structure for 1.5 seconds to preview its function.</p><p>Click to pin its label. Press Escape to clear.</p></div>}</div></section>
 <section className="structure-nav"><div className="nav-heading"><h2><span>//</span> STRUCTURE INDEX</h2><span><MousePointer2 size={14}/> Select a structure to lock its label</span></div><div className="parts">{parts.map((part,i)=><button key={part.name} aria-pressed={pinned===i} onClick={()=>{anchor.current=null;pin(i);}} className={selected===i?'active':''} style={{'--part-color':part.color} as React.CSSProperties}><span className="index">0{i+1}</span><i/>{part.name}<span className="target-indicator">{pinned===i?'+':'⌁'}</span></button>)}</div></section>
 <footer><span>TEACHING MODEL / Colors distinguish structures / Not to scale</span><a href="https://www.ncbi.nlm.nih.gov/books/NBK11103/" target="_blank" rel="noreferrer">Neuroscience reference <ArrowUpRight size={14}/></a></footer>
 </main>}
